...
**Domain:** `Supply Chain / Warehouse` · **Suite:** `Supply Chain Suite` · **Accent:** `#fdba74`
...
**Delivery Note Parser** is a standalone, browser-based AI workflow app for **Supply Chain / Warehouse**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `delivery_note`<br>Delivery note | text | Yes | — | Affects input: Delivery note. |
...
| `warehouse`<br>warehouse | text | No | — | Affects customization: warehouse. |
| `po_source`<br>PO source | text | No | — | Affects customization: PO source. |
| `unit_rules`<br>unit rules | textarea | No | — | Affects customization: unit rules. |
| `tolerance`<br>tolerance | slider | No | 50 | Affects customization: tolerance. |
| `supplier_profile`<br>supplier profile | select | No | — | Affects customization: supplier profile. |
| `currency`<br>currency | select | No | USD | Affects customization: currency. |
| `approval_policy`<br>approval policy | text | No | — | Affects customization: approval policy. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9116
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9116`. |
...
```bash
curl http://127.0.0.1:9116/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9116/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9116 uvicorn app.main:app --host 0.0.0.0 --port 9116
```
...
- `plugins/delivery-note-parser.json` for plugin metadata.
